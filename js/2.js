$(function(){
    ...
    var musicImgsData = ['imge/ge1.png','imge/bg1.jpg','imge/bg2.jpg','imge/bg.jpg','imge/ge2.png'] ;   // 图片地址数组
    var musicNameData = ["醉赤壁","老人与海","是想你的声音啊","不分手的恋爱","初雪告白"];                   // 歌曲名数组
    var artistNameData = ['林俊杰','海鸣威','傲七爷','汪苏泷','麓七'] ;           // 创作歌手数组
    var musicUrls=['mp3/林俊杰 - 醉赤壁.mp3','mp3/海鸣威-老人与海.mp3','mp3/是想你的声音啊-傲七爷.mp3','mp3/不分手的恋爱-汪苏泷silence.w.mp3','mp3/初雪告白-麓七.mp3'];// 歌曲mp3数组
...
for(var i=0;i<len;i++){
    ...
    var txt1= document.createTextNode(musicNameData[i]);
    var txt2= document.createTextNode(artistNameData[i]);
    ge.appendChild(li);
    a.appendChild(txt1);
...
var b = ge.getElementsByTagName('li');
     ...
     var rrr = tab.getElementsByTagName('tr');
     for(var i=1;i<=tab.length;i++){
        (function (i){
      rrr[i].onclick=function(){
        currIndex=i;
        musicImgs.removeClass('buffering');  
        playPauseBtn.attr('class','btn play-pause icon-zanting iconfont') // 显示暂停图标
        seekBar.width(0);           // 重置播放进度条为0
